...
public:
    struct KbdMapping
    {
        ...
        KbdMapping() : scancode(0), x11Keycode(0), isMouseButton(false) {}
    };
    ...
    explicit ProgramOptions(const string& programName, int lineLength=78)
        ...
        , m__envvarOpts(lineLength)
        , m__cfgfileOpts("Configuration File Variables", lineLength)
          // The extra offset of 4, below, accounts for the space added by
          // boost::program_options for the fake option.
        , m__cfgfileDocDetails("Details",
                               lineLength + m__B_PO_MIN_LEFT_MARGIN - 4)
    {
...
void ProgramOptions::defineCfgfileOptions()
{
    ...
    m__cfgfileOpts.add_options()
        ...
        ("ZoomUp.isMouseButton",
         ...
         "When set to 'true', the value specified to \"ZoomUp.x11Keycode\" is"
...
void ProgramOptions::showConfigHelp(const b_po_opt_descr_t& config_descr)
{
    ...
    while(configDoc_sst) {
        ...
        if(line.find("--") == leftMargin) {
            ...
            leftMargin = m__B_PO_MIN_LEFT_MARGIN;
        }
        // Ignore lines that haven't been indented.
        if (m__B_PO_MIN_LEFT_MARGIN <= leftMargin) {
            line.erase(0, m__B_PO_MIN_LEFT_MARGIN);
        }
